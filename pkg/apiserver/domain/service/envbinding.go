...
type EnvBindingService interface {
	...
	ApplicationNormalEnvRecycle(context.Context, apisv1.CreateApplicationRequest, string) error
}
...
func (e *envBindingServiceImpl) ApplicationNormalEnvRecycle(ctx context.Context, req apisv1.CreateApplicationRequest, envBinding string) error {
	var app v1beta1.Application
	err := e.KubeClient.Get(ctx, types.NamespacedName{Namespace: "default", Name: req.Name}, &app)
	if err != nil {
		if apierrors.IsNotFound(err) {
			return nil
		}
