...
type ClusterService interface {
	...
	ListClusterNamespace(context.Context, string) (*apis.ListClusterNamespaceResponse, error)
}
...
func (c *clusterServiceImpl) ListClusterNamespace(ctx context.Context, clusterName string) (*apis.ListClusterNamespaceResponse, error) {
	// just to check the cluster in or not in
	_, err := c.getClusterFromDataStore(ctx, clusterName)
	if err != nil {
		if errors.Is(err, datastore.ErrRecordNotExist) {
			return nil, bcode.ErrClusterNotFoundInDataStore
	...
	// to check the cluster namespace

	//var namespaces corev1.NamespaceList
	namespaces := &corev1.NamespaceList{}
	if err = c.K8sClient.List(multicluster.ContextWithClusterName(ctx, clusterName), namespaces); err != nil {
		if kerrors.IsForbidden(err) {
			return nil, bcode.ErrClusterCreateNamespaceNoPermission
		}
		return nil, errors.Wrapf(err, "failed to list namespace in cluster %s", clusterName)
	}
	var nses []string
	for _, namespace := range namespaces.Items {
		var ns = namespace.GetName()
		nses = append(nses, ns)

	}
	return &apis.ListClusterNamespaceResponse{Cluster: clusterName, Namespaces: nses}, nil
}
