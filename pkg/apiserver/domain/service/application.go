...
func (c *applicationServiceImpl) ListApplications(ctx context.Context, listOptions apisv1.ListApplicationOptions) ([]*apisv1.ApplicationBase, error) {
	...
	*/
	projects, err := c.ProjectService.ListUserProjects(ctx, "admin")
	if err != nil {
		return nil, err
	}
...
func (c *applicationServiceImpl) CreateNormalApplication(ctx context.Context, app v1beta1.Application) (*apisv1.CreateNormalResponse, error) {

	err := c.KubeClient.Get(ctx, types.NamespacedName{Namespace: "default", Name: app.Name}, &app)
	if err != nil {
		if apierrors.IsNotFound(err) {
			err := c.Apply.Apply(ctx, &app)
			if err != nil {
				return nil, err
			}
			var base = &apisv1.CreateNormalResponse{
				Name:      app.Name,
