...
func (c *applicationAPIInterface) GetWebServiceRoute() *restful.WebService {
	...
	ws.Route(ws.POST("/install").To(c.createNormalApplication).
		Doc("create normal applications ").
		Metadata(restfulspec.KeyOpenAPITags, tags).
		Reads(v1beta1.Application{}).
		Filter(c.RbacService.CheckPerm("application", "create")).
		Returns(200, "OK", apis.CreateNormalResponse{}).
		Returns(400, "Bad Request", bcode.Bcode{}).
		Writes(apis.CreateNormalResponse{}))

	ws.Route(ws.DELETE("/{appName}/uninstall").To(c.deleteNormalApplications).
		Doc("delete normal application").
		Metadata(restfulspec.KeyOpenAPITags, tags).
		Filter(c.RbacService.CheckPerm("application", "delete")).
		Filter(c.appCheckFilter).
		Param(ws.PathParameter("appName", "identifier of the application ").DataType("string").Required(true)).
...
func (c *applicationAPIInterface) createNormalApplication(req *restful.Request, res *restful.Response) {
	var app v1beta1.Application
	con, err := ioutil.ReadAll(req.Request.Body)
	if err != nil {
		bcode.ReturnError(req, res, err)
		return
	}
	...
	err = json.Unmarshal(con, &jsonMap)
	if err != nil {
		bcode.ReturnError(req, res, err)
		return
	}
	_, ok := jsonMap["kind"]
	if !ok {
		//err := error("Properties required:Req.body.Kind")
		bcode.ReturnError(req, res, bcode.ErrApplicationBodyProperties)
		return
	}
	app.Kind = jsonMap["kind"]

	_, ok = jsonMap["apiVersion"]
	if !ok {
		bcode.ReturnError(req, res, bcode.ErrApplicationBodyProperties)
		return
	}
	app.APIVersion = jsonMap["apiVersion"]

	_, ok = jsonMap["metadata"]
	if !ok {
		bcode.ReturnError(req, res, bcode.ErrApplicationBodyProperties)
		return
	}
	...
	err = json.Unmarshal([]byte(md), &app.ObjectMeta)
	if err != nil {
		bcode.ReturnError(req, res, err)
		return
	}
	...
	spec := jsonMap["spec"]
	strings.TrimLeft(spec, "{")
	strings.TrimRight(spec, "}")
	err = json.Unmarshal([]byte(spec), &app.Spec)
	if err != nil {
		bcode.ReturnError(req, res, err)
		return
	}
	appBase, err := c.ApplicationService.CreateNormalApplication(req.Request.Context(), app)
	if err != nil {
		bcode.ReturnError(req, res, err)
		return
	}
...
func (c *applicationAPIInterface) deleteNormalApplications(req *restful.Request, res *restful.Response) {
	var createReq apis.CreateApplicationRequest
	createReq.Name = req.PathParameter("appName")
	createReq.Project = "default"
	env := "default"
	err := c.EnvBindingService.ApplicationNormalEnvRecycle(req.Request.Context(), createReq, env)
	if err != nil {
		bcode.ReturnError(req, res, err)
		return
	}
