...
func (c *ClusterAPIInterface) GetWebServiceRoute() *restful.WebService {
	...
	ws.Route(ws.GET("/{clusterName}/namespaces").To(c.ListClusterNamespace).
		Doc("list namespace in cluster").
		Metadata(restfulspec.KeyOpenAPITags, tags).
		Param(ws.PathParameter("clusterName", "name of the target cluster").DataType("string")).
		Filter(c.RbacService.CheckPerm("cluster/namespace", "list")).
		Returns(200, "OK", apis.ListClusterNamespaceResponse{}).
		Returns(400, "Bad Request", bcode.Bcode{}).
		Writes(apis.ListClusterNamespaceResponse{}))
...
func (c *ClusterAPIInterface) ListClusterNamespace(req *restful.Request, res *restful.Response) {
	clusterName := req.PathParameter("clusterName")

	// Call the domain layer code
	resp, err := c.ClusterService.ListClusterNamespace(req.Request.Context(), clusterName)
	if err != nil {
		bcode.ReturnError(req, res, err)
		return
	}
